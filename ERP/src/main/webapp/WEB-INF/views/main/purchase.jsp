...
<!-- empinfo CSS 적용 -->
<link rel="stylesheet"
	href="../../../resources/css/purchase.css?version=1.8">
<!-- empinfo js -->
<script type="text/javascript"
	src="../../../resources/js/purchase.js?version=1.8"></script>
<script type="text/javascript">
	function delchk() {
						...
						<div class="body_left">
							...
							<div class="button_div">
								<input type="submit" id="signup" value="등록" class="submit"
									...
									type="button" value="취소" class="cancle"></a>
							</div>
						</div>
					</form>
					...
					<div class="body_right">
						...
						<div class="page">
							...
							<c:if test="${paging.prev}">
								<a class="pagemove"
									href="/main/purchase?type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${paging.startPage-1}&amount=${paging.cri.amount}"><</a>
							</c:if>
							...
							<c:forEach begin="${paging.startPage}" end="${paging.endPage}"
								...
								<c:choose>
									<c:when test="${paging.cri.pageNum==num}">
										<a class="selectpage"
											href="/main/purchase?type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${num}&amount=${paging.cri.amount}">${num}</a>
									</c:when>
									<c:otherwise>
										<a class="pagenum"
											href="/main/purchase?type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${num}&amount=${paging.cri.amount}">${num}</a>
							...
							<c:if test="${paging.next}">
								<a class="pagemove"
									href="/main/purchase?type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${paging.endPage+1}&amount=${paging.cri.amount}">></a>
							</c:if>
						</div>
						<!-- page div -->
					</div>
