...
<!-- empinfo js -->
<script type="text/javascript"
	src="../../../resources/js/purchase_all_list.js?version=2.0"></script>
<script type="text/javascript">
	function delchk() {
					...
					<div class="body_right">
						...
						<div class="page">
							...
							<c:if test="${paging.prev}">
								<a class="pagemove"
									href="/main/purchase_all_list?type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${paging.startPage-1}&amount=${paging.cri.amount}"><</a>
							</c:if>
							...
							<c:forEach begin="${paging.startPage}" end="${paging.endPage}"
								...
								<c:choose>
									<c:when test="${paging.cri.pageNum==num}">
										<a class="selectpage"
											href="/main/purchase_all_list?type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${num}&amount=${paging.cri.amount}">${num}</a>
									</c:when>
									<c:otherwise>
										<a class="pagenum"
											href="/main/purchase_all_list?type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${num}&amount=${paging.cri.amount}">${num}</a>
							...
							<c:if test="${paging.next}">
								<a class="pagemove"
									href="/main/purchase_all_list?type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${paging.endPage+1}&amount=${paging.cri.amount}">></a>
							</c:if>
						</div>
						<!-- page div -->
					</div>
